...
namespace Servers
{
  internal class StorageServer
  {
    ...
    public StorageServer()
    {
      try
      {
        if (Config.LogMessages)
          Console.WriteLine("[StorageServer] has started.");
    ...
    {
      this.listener.Prefixes.Add(string.Format("http://+:{0}/", (object) Config.storageServerPort));
      while (true)
      {
        this.listener.Start();
        if (Config.LogMessages)
          Console.WriteLine("[StorageServer] is listening for a request.");
